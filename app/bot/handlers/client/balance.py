...
@router.message(
    ButtonFilter(Buttons.BALANCE), or_f(ClientState.main_menu, ClientState.balance_menu)
)
...
async def get_key(
    ...
    while flag.get("waiting_payment") and try_ < max_tries:
        if await transaction_is_completed(transaction_id):
            ...
            await bot.send_message(
                ...
                f"Баланс пополнен на сумму: {new_sum} руб.",
    ...
    if flag.get("waiting_payment"):
        ...
        await bot.send_message(
            message.from_user.id,
            "Оплата не прошла! Время ожидания истекло.",
            reply_markup=balance_keyboard,
        )
        logger.warning(
...
async def cancel_payment(query: CallbackQuery, bot: Bot, state: FSMContext, redis: Redis):
    ...
    await bot.send_message(
        query.message.chat.id, "Оплата отменена.", reply_markup=balance_keyboard
    )
