...
from modules.database.methods.users import add_user, get_user_by_tg_id, get_all_users, change_user_role
...
async def create_user(user: UserCreate, session: AsyncSession = Depends(get_async_session)):
    ...
    """
    try:
        new_user = await add_user(tg_id=user.tg_id, role_id=user.role_id, username=user.username, session=session)
...
async def change_role(tg_id: int, role_change: RoleChange, session: AsyncSession = Depends(get_async_session)):
    """
    Изменяет роль пользователя.
    """
    user = await change_user_role(tg_id=tg_id, new_role_id=role_change.new_role_id, session=session)
    if not user:
        raise HTTPException(status_code=404, detail="Пользователь не найден.")
    return user
